...
export class GameScene extends Phaser.Scene {
    ...
    startRound() {
        ...
        this.time.delayedCall(Phaser.Math.Between(100, 3000), () => {
            // Retraso aleatorio entre 0.1 y 3 segundos
    ...
    handleInput(index) {
        ...
        AudioManager.playSFX(
            this,
            [ASSET_KEYS.AUDIO.SFX_ROCK, ASSET_KEYS.AUDIO.SFX_PAPER, ASSET_KEYS.AUDIO.SFX_SCISSORS][
                index
            ]
        );
    ...
    showResults(p1, p2) {
        ...
        this.ui.showRoundResult(
            ...
            () => {
                ...
                if (
                    outcome.result === 'WIN' &&
                    CampaignManager.isActive() &&
                    CampaignManager.state.currentLevel === 3 &&
                    CampaignManager.state.winsInRow === 1
                ) {
                ...
                if (this.p2Health <= 0) {
                    this.time.delayedCall(500, () =>
                        this.triggerFatality(p1, this.p2Emoji, this.p1Emoji, true)
                    );
                } else if (this.p1Health <= 0) {
                    this.time.delayedCall(500, () =>
                        this.triggerFatality(p2, this.p1Emoji, this.p2Emoji, false)
                    );
                } else {
    ...
    triggerFatality(choiceIndex, target, attacker, isPlayerWin) {
        ...
        // Vibrate for fatality impact
        if (DataManager.isVibrateEnabled() && navigator.vibrate) {
        ...
        this.tweens.add({
            targets: [
                this.p1Status,
                this.p2Status,
                this.ui.elements.p1Hearts,
                this.ui.elements.p2Hearts,
                this.timeText1,
                this.timeText2,
            ],
            alpha: 0,
        ...
        this.fatalityManager.play(choiceIndex, target, attacker, isPlayerWin, (winState) =>
            this.finishGame(winState)
        );
    }
    ...
    async finishGame(isPlayerWin) {
        ...
        if (!CampaignManager.isActive()) {
            // Solo contar racha en Quick Play
            if (isPlayerWin) {
    ...
    makeSingleBtn(x, y, name, emoji, isP1, index) {
        ...
        const bg = this.add
            .circle(0, 0, 38, 0x000000, 0.1)
            .setStrokeStyle(CONFIG.UI.BORDER_WIDTH, borderColor);

        const emo = this.add
            .text(0, 0, emoji, { fontSize: CONFIG.FONTS.SIZES.EMOJI, padding: { x: 10, y: 10 } })
            .setOrigin(0.5);
    ...
    switchSides() {
        ...
        this.tweens.add({
            targets: [
                this.ui.elements.p1Profile,
                this.ui.elements.p1Avatar,
                this.ui.elements.p1Hearts,
                this.ui.elements.p1Emoji,
                this.ui.elements.p1X,
            ],
            x: p1TargetX,
            duration: 400,
            ease: 'Power2',
        ...
        if (!this.isPlayerRight) {
            ...
            // La máquina está a la izquierda (this.isPlayerRight), el indicador va encima del configBtn
            indicatorBtnRef = this.ui.elements.configBtn;
        }
        // Posicionar encima del centro del botón, con offset vertical para que esté "arriba"
        ...
        this.tweens.add({
            targets: [
                this.ui.elements.p2Profile,
                this.ui.elements.p2Avatar,
                this.ui.elements.p2Hearts,
                this.ui.elements.p2Emoji,
                this.ui.elements.p2X,
            ],
            x: p2TargetX,
            duration: 400,
            ease: 'Power2',
        ...
        this.tweens.add({
            targets: this.ui.elements.aiIndicator,
            x: aiIndicatorTargetX,
            y: aiIndicatorTargetY,
            duration: 400,
            ease: 'Power2',
        }); // Mover también en Y
        ...
        this.p1Buttons.forEach((btn) => {
            if (btn) this.tweens.add({ targets: btn, x: p1X, duration: 400, ease: 'Back.easeOut' });
        });
        this.p2Buttons.forEach((btn) => {
            if (btn) this.tweens.add({ targets: btn, x: p2X, duration: 400, ease: 'Back.easeOut' });
        });
        this.tweens.add({
            targets: this.timeText1,
            x: LAYOUT.getColumnX(width, true),
            duration: 400,
            ease: 'Power2',
        });
        this.tweens.add({
            targets: this.timeText2,
            x: LAYOUT.getColumnX(width, false),
            duration: 400,
            ease: 'Power2',
        });
    ...
    getCpuChoice() {
        return OpponentAI.getChoice(this.difficulty, this.playerStats);
    }
    ...
    getDialoguePhrase(speaker, type) {
        ...
        if (!this.dialogueDecks[speaker][type] || this.dialogueDecks[speaker][type].length === 0) {
            this.dialogueDecks[speaker][type] = Phaser.Utils.Array.Shuffle([
                ...DIALOGUES[speaker][type],
            ]);
        }
    ...
    pauseGame() {
        ...
        const bg = this.add
            .rectangle(0, 0, width, height, CONFIG.COLORS.BG_DARK, 0.7)
            .setOrigin(0)
            .setInteractive();
        const txt = this.add
        ...
        const subTxt = this.add
            .text(width / 2, height / 2 + 60, 'TAP TO RESUME', {
                fontFamily: CONFIG.FONTS.MAIN,
                fontSize: CONFIG.FONTS.SIZES.NORMAL,
                fill: CONFIG.COLORS.P1_BLUE,
            })
            .setOrigin(0.5);
