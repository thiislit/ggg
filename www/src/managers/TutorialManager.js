...
export class TutorialManager {
    ...
    showTutorial() {
        ...
        const overlay = this.scene.add
            .rectangle(0, 0, width, height, CONFIG.COLORS.BG_DARK, 0.8)
            .setOrigin(0)
            .setInteractive();
        const style = {
        ...
        const text = this.scene.add
            .text(width / 2, height * 0.4, 'CHOOSE YOUR WEAPON\nBEFORE TIME RUNS OUT!', style)
            .setOrigin(0.5);
        const btn = this.scene.add.container(width / 2, height * 0.6);
        const bRect = this.scene.add
            .rectangle(0, 0, 200, 60, CONFIG.COLORS.P1_BLUE)
            .setStrokeStyle(2, 0xffffff)
            .setInteractive({ useHandCursor: true });
        const bText = this.scene.add
            .text(0, 0, 'GOT IT!', { fontFamily: CONFIG.FONTS.MAIN, fontSize: '16px' })
            .setOrigin(0.5);
        btn.add([bRect, bText]);
        ...
        const handEmoji = this.scene.add
            .text(width / 2, height * 0.8, '👆', { fontSize: '60px' })
            .setOrigin(0.5);
